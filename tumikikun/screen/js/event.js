...
let moji = '';
    ...
    modalOpen.addEventListener('click', () =>{
        let imageurl = 'https://halproducts.main.jp/testUpdate/U22_web/img.png';
        let postdata = '{"Url":"' + imageurl + '"}'; 
        $.ajax({
            type: 'POST',
            url: 'https://haloh22cvnt21furuyama-prediction.cognitiveservices.azure.com/customvision/v3.0/Prediction/9e2fa579-5f7b-4f33-8e5f-a4db0f3fbe4e/detect/iterations/Iteration37/url',
            data: postdata,
            headers: {
                "Prediction-Key": "*******",
                "Content-Type": "******"
            ...
            success: function(data) {
                ...
                // // テストデータ
                // block_data = [];
                // block_data[0] = ['y_square', 97.08806, 0.00012605];
                // block_data[1] = ['square', 89.37148499999999, 0.01268564];
                // block_data[2] = ['r_rectangle', 89.37148499999999, 0.10268564];
                // block_data[3] = ['y_square', 91.67300499999999, 0.15317202];
                // block_data[4] = ['r_rectangle', 89.37148499999999, 0.2268564];
                // block_data[5] = ['b_rectangle', 95.28469000000001, 0.34407774];
                // block_data[6] = ['rectangle', 90.29359, 0.5538967];
                // block_data[7] = ['square', 91.93123, 0.62863064];
                // block_data[8] = ['rectangle', 82.62743, 0.70447093];
                // block_data[9] = ['y_square', 97.08806, 0.7872605];
    ...
    const modalClose4 = document.getElementById('openbutton');
        modalClose4.addEventListener('click', () =>{
            ...
            if(flag == 1){//ループが入っている時
                ...
                //             layout.innerHTML +="<div class='buttoncon'><li class='pulse-btn'><img src='./img/nor2.png'></li><div id='down_nor'>-</div><div id='text_nor'>"+nor_cntxt+"</div><div id='up_nor'>+</div></div>";
                //             var nor_flg="in";
                //         }
                //     }
                ...
                for (let i = 0; i < blocks.length; i++) {//要素分まで実行される
                    if(blocks[i][0] == "rectangle"){ //木のループの時
                        nor_cnt++;
                        if(nor_cnt == 1){
                            layout.innerHTML +="<div class='buttoncon'><li class='pulse-btn'><img src='./img/nor2.png'></li><div class='loopTsumiki'><div id='up_nor'><img src='./title_backimg/up.png' class='upImg'></div><div class='count_num'><p id='text_nor'>"+nor_cntxt+"</p></div><div id='down_nor'><img src='./title_backimg/down.png' class='downImg'></div></div></div>";
                        }
                        else{
                            layout.innerHTML +="<li class='pulse-btn'><img src='./img/nor2.png'></li>";
                            var nor_flg="in";
                        }
                    }
                    else if(blocks[i][0] == "y_square"){ //黄色の四角
                        layout.innerHTML +="<li><img src='./img/y.png'></li>";
                    }
                    else if(blocks[i][0] == "b_rectangle"){ //青のジャンプ
                        layout.innerHTML +="<li><img src='./img/b.png'></li>";
                    }
                    else if(blocks[i][0] == "r_rectangle"){ //赤のループ
                        red_cnt++;
                        if(red_cnt == 1){
                            layout.innerHTML +="<div class='buttoncon'><li class='pulse-btn'><img src='./img/red.png'></li><div class='loopTsumiki'><div id='up_red'><img src='./title_backimg/up.png' class='upImg'></div><div class='count_num'><p id='text_red'>"+red_cntxt+"</p></div><div id='down_red'><img src='./title_backimg/down.png' class='downImg'></div></div></div>";
                        }
                        else{
                            layout.innerHTML +="<li class='pulse-btn'><img src='./img/red.png'></li>";
                            var red_flg="in";
                        }
                    }
                    else if(blocks[i][0] == "x_rectangle"){ //第三のループ
                        x_cnt++;
                        if(x_cnt == 1){
                            layout.innerHTML +="<div class='buttoncon'><li class='pulse-btn'><img src='./img/x.png'></li><div class='loopTsumiki'><div id='up_x'><img src='./title_backimg/up.png' class='upImg'></div><div class='count_num'><p id='text_x'>"+x_cntxt+"</p></div><div id='down_x'><img src='./title_backimg/down.png' class='downImg'></div></div></div>";
                        }
                        else{
                            layout.innerHTML +="<li class='pulse-btn'><img src='./img/x.png'></li>";
                            var x_flg="in";
                        }
                    }
                    else{
                        layout.innerHTML +="<li><img src='./img/nor.png'></li>";
                    }
                }
            ...
            success.addEventListener('click', () =>{
                ...
                if(cnt[0] != 0){
                    ...
                    for(let val of block_data){
                        ...
                        else if(depth_cnt === 0 && loop.length === 0){
                            process.push(val[0]);
                        }
                ...
                // 二重ループ
                    ...
                    for(let val of block_data){
                        ...
                        // 外側ループ開始位置と内側ループ開始位置の間の処理を格納
                        else if(depth_cnt === 1 && loop.length === 0){
                            loop_front.push(val[0]);   
                        }
                        // 外側ループ終了位置と内側ループ終了位置の間の処理を格納
                        else if(depth_cnt === 1 && loop.length != 0){
                    ...
                    // 上で作った配列をprocessにまとめる
                    for(let i=0; i<cnt[2][0]; i++){
                        for(let j=0; j<loop_front.length; j++){
                            process.push(loop_front[j]);
                        }
                        for(let m=0; m<cnt[2][1]; m++){
                            for(let k=0; k<loop.length; k++){
                                process.push(loop[k]);
                            }
                        }
                        for(let l=0; l<loop_rear.length; l++){
                            process.push(loop_rear[l]);
                        }
                    }
                    for(let i=0; i<rear.length; i++){
                        process.push(rear[i]);
        ...
        go.addEventListener('click', () =>{
            ...
            for(let i = 0; i < block_data.length;i++){
                if(NumberMozi.length === 0){
                    NumberMozi = block_data[i][0];
                }
                else{
                    // console.log(block_data[0][0]);
                    NumberMozi = NumberMozi + ',' + block_data[i][0];
                }
            }
            ...
            console.log(mozi);
            // console.log(NumberMozi);
            console.log(NumberMozi);
            document.getElementById( "json" ).value = mozi ;
            // const aaaa = document.getElementById("numberCount").value;
            // console.log(aaaa);
            document.getElementById("numberCount").value = NumberMozi;
